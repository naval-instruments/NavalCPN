        ...
        for( unsigned int i = 0; i < file_array.GetCount(); i++ ) {
            ...
            if( ::wxFileExists( path ) ) {
                ...
                if(islayer){
                    ...
                    if(wpt_dups > 0) {
                        OCPNMessageBox(parent, wxString::Format(_T("%d ")+_("duplicate waypoints detected during import and ignored."), wpt_dups), _("NavalCPN Info"), wxICON_INFORMATION|wxOK, 10);
                    }
                }
                delete pSet;
            }
