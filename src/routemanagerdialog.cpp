...
void RouteManagerDialog::OnRteDeleteClick( wxCommandEvent &event )
{
    ...
    int answer = OCPNMessageBox( this, _("Are you sure you want to delete the selected object(s)"), wxString( _("NavalCPN Alert") ), wxYES_NO );
    if ( answer != wxID_YES )
        return;
...
void RouteManagerDialog::OnRteDeleteAllClick( wxCommandEvent &event )
{
    int dialog_ret = OCPNMessageBox( this, _("Are you sure you want to delete <ALL> routes?"),
            wxString( _("NavalCPN Alert") ), wxYES_NO );
    ...
    switch( event.GetId() ) {
        ...
        case TRACK_CLEAN: {
            ...
            wxString msg = wxString::Format( _("The amount of data used by the track\n was reduced by %d%%."),
                    reduction );
            OCPNMessageBox( this, msg, _("NavalCPN info"), wxICON_INFORMATION | wxOK );
...
void RouteManagerDialog::TrackToRoute( Track *track )
{
    ...
    wxGenericProgressDialog pprog( _("NavalCPN Converting Track to Route...."),
    ...
    if( busy ) {
        for(unsigned int i=0 ; i < list.GetCount() ; i++) {
            ...
            if( wp ) {
                ...
                if ( wp->m_bIsInRoute )
                {
                    if ( wxID_YES == OCPNMessageBox(this,  _( "The waypoint you want to delete is used in a route, do you really want to delete it?" ), _( "NavalCPN Alert" ), wxYES_NO ))
...
void RouteManagerDialog::OnWptDeleteAllClick( wxCommandEvent &event )
{
    ...
    int answer = OCPNMessageBox( this, prompt, wxString( _("NavalCPN Alert") ), buttons );
    if ( answer == wxID_YES )
...
void RouteManagerDialog::OnLayDeleteClick( wxCommandEvent &event )
{
    ...
    {
        prompt.Append( _T("\n") );
        prompt.Append( _("The file will also be deleted from NavalCPN's layer directory.") );
        prompt.Append( _T("\n (") +  destf + _T(")" ) );
        ispers = true;
    }
    int answer = OCPNMessageBox( this, prompt, wxString( _("NavalCPN Alert") ), wxYES_NO );
    if ( answer == wxID_NO )
        return;
