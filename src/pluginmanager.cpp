...
bool PlugInManager::LoadPlugInDirectory(const wxString &plugin_dir, bool load_enabled, bool b_enable_blackdialog)
{
    ...
    for(unsigned int i=0 ; i < file_list.GetCount() ; i++) {
        ...
        if(m_benable_blackdialog && !b_compat)
        {
            ...
            OCPNMessageBox( NULL, wxString::Format(_("The plugin %s is not compatible with this version of NavalCPN, please get an updated version."), plugin_file.c_str()), wxString(_("NavalCPN Info")), wxICON_INFORMATION | wxOK, 10 );
        }
...
void PlugInManager::ShowDeferredBlacklistMessages()
{
    for( unsigned int i=0 ; i < m_deferred_blacklist_messages.GetCount() ; i++){
        OCPNMessageBox ( NULL, m_deferred_blacklist_messages[i], wxString( _("NavalCPN Info") ), wxICON_INFORMATION | wxOK, 5 );  // 5 second timeout
    }
    ...
    for (int i = 0; i < len; i++) {
            ...
            ( !PluginBlacklist[i].all_lower && name == PluginBlacklist[i].name && PluginBlacklist[i].version_major == major && PluginBlacklist[i].version_minor == minor ) )
        {
            ...
            if(m_benable_blackdialog)
                OCPNMessageBox ( NULL, msg, wxString( _("NavalCPN Info") ), wxICON_INFORMATION | wxOK, 5 );  // 5 second timeout
            else
...
PlugInContainer *PlugInManager::LoadPlugIn(wxString plugin_file)
{
    ...
    if( m_benable_blackdialog && !wxIsReadable(plugin_file) )
    {
        ...
        msg += plugin_file;
        msg += _T("\n\n");
        OCPNMessageBox ( NULL, msg, wxString( _("NavalCPN Info") ), wxICON_INFORMATION | wxOK, 10 );  // 10 second timeout
    }
        ...
        //  extract the probable plugin name
            ...
            for (int i = 0; i < len; i++) {
                ...
                if( prob_pi_name.Lower().EndsWith(candidate)){
                    ...
                    msg += _(" is incompatible with this version of NavalCPN."),
...
void PlugInManager::SendBaseConfigToAllPlugIns()
{
    ...
    wxJSONValue v;
    v[_T("NavalCPN Version Major")] = VERSION_MAJOR;
    v[_T("NavalCPN Version Minor")] = VERSION_MINOR;
    v[_T("NavalCPN Version Patch")] = VERSION_PATCH;
    v[_T("NavalCPN Version Date")] = VERSION_DATE;
    v[_T("NavalCPN Version Full")] = VERSION_FULL;

    // Some useful display metrics
    if(g_MainToolbar){
        v[_T("NavalCPN Toolbar Width")] = g_MainToolbar->GetSize().x;
        v[_T("NavalCPN Toolbar Height")] = g_MainToolbar->GetSize().y;
        v[_T("NavalCPN Toolbar PosnX")] = g_MainToolbar->GetPosition().x;
        v[_T("NavalCPN Toolbar PosnY")] = g_MainToolbar->GetPosition().y;
    }

    // Some rendering parameters
    v[_T("NavalCPN Zoom Mod Vector")] = g_chart_zoom_modifier_vector;
    v[_T("NavalCPN Zoom Mod Other")] = g_chart_zoom_modifier;
    v[_T("NavalCPN Display Width")] = (int)g_display_size_mm;

    wxJSONWriter w;
    wxString out;
    w.Write(v, out);
    SendMessageToAllPlugins(wxString(_T("NavalCPN Config")), out);
}
...
//         v[_T("OpenCPN S52PLIB DisplayCategory")] = ps52plib->GetDisplayCategory();
        ...
        v[_T("NavalCPN S52PLIB DeclutterText")] = ps52plib->m_bDeClutterText;
        v[_T("NavalCPN S52PLIB ShowNationalText")] = ps52plib->m_bShowNationalTexts;
        v[_T("NavalCPN S52PLIB ShowImportantTextOnly")] = ps52plib->m_bShowS57ImportantTextOnly;
        v[_T("NavalCPN S52PLIB UseSCAMIN")] = ps52plib->m_bUseSCAMIN;
        v[_T("NavalCPN S52PLIB SymbolStyle")] = ps52plib->m_nSymbolStyle;
        v[_T("NavalCPN S52PLIB BoundaryStyle")] = ps52plib->m_nBoundaryStyle;
        v[_T("NavalCPN S52PLIB ColorShades")] = S52_getMarinerParam( S52_MAR_TWO_SHADES );
    }

    // Notify plugins that S52PLIB may have reconfigured global options
    v[_T("NavalCPN S52PLIB GlobalReconfig")] = bReconfig;
...
{
    return _T("NavalCPN PlugIn Base Class");
}
