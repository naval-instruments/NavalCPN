...
void ChartCanvas::SetGroupIndex( int index, bool autoSwitch )
{
    ...
    if(!autoSwitch && bgroup_override){
        ...
        OCPNMessageBox( this, msg, _("NavalCPN Group Notice"), wxICON_INFORMATION, 2 );
    ...
    if( bgroup_override ) {
            ...
            OCPNMessageBox( this, msg, _("NavalCPN Group Notice"), wxOK, 5 );
    }
}
                ...
                if(m_pMouseRoute){
                    if( m_routeState == 1 ) {
                        ...
                        if( m_pMouseRoute->m_NextLegGreatCircle ) {
                            ...
                            int answer = OCPNMessageBox( this, msg, _("NavalCPN Route Create"), wxYES_NO | wxNO_DEFAULT );
...
void ChartCanvas::UpdateCanvasS52PLIBConfig()
{
    ...
    if(bSendPlibState){
        ...
        //  S52PLIB state
        v[_T("NavalCPN S52PLIB ShowText")] = GetShowENCText();
        v[_T("NavalCPN S52PLIB ShowSoundings")] = GetShowENCDepth();
        v[_T("NavalCPN S52PLIB ShowLights")] = GetShowENCLights();
        v[_T("NavalCPN S52PLIB ShowAnchorConditions")] = m_encShowAnchor; //ps52plib->GetAnchorOn();
        v[_T("NavalCPN S52PLIB ShowQualityOfData")] = GetShowENCDataQual(); //ps52plib->GetQualityOfDataOn();
        v[_T("NavalCPN S52PLIB ShowATONLabel")] = GetShowENCBuoyLabels();
        v[_T("NavalCPN S52PLIB ShowLightDescription")] = GetShowENCLightDesc();

        v[_T("NavalCPN S52PLIB DisplayCategory")] = GetENCDisplayCategory();
...
*/
        ...
        if(!g_lastPluginMessage.IsSameAs(out)){
            //printf("message %d  %d\n", s_msg++, m_canvasIndex);
            g_pi_manager->SendMessageToAllPlugins(wxString(_T("NavalCPN Config")), out);
        }
    }
}
int spaint;
